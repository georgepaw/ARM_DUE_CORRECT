...
#define INSTRUCTIONS_PER_CRC 1024
...
public:
  ...
  SECDED instructions[INSTRUCTIONS_PER_CRC];
  uint32_t num_instructions_filled;
  ...
  void clean();
  void add_instruction(uint32_t instruction);
  void change_instruction(uint32_t index, SECDED secded);
  uint32_t generate_crc();
  bool check();
  uint32_t get_num_instructions() { return num_instructions_filled; }
};